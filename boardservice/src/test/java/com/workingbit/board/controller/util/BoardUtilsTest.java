...
//    String notation = squareDouble1Array.stream().map(ICoordinates::getNotationDrives).collect(Collectors.joining(","));
//    assertEquals("h8,g7,f6,e5,d4,c3,b2,a1", notation);
//  }
//
//  @Test
//  public void test_double_diagonals_main() {
...
//    notation = squareDouble2Array.stream().map(ICoordinates::getNotationDrives).collect(Collectors.joining(","));
//    assertEquals("b8,c7,d6,e5,f4,g3,h2", notation);
//  }
//
//  @Test
//  public void test_triple_diagonals_sub() {
...
//    String stringStream = mainDiagonals.stream().map(squares -> squares.stream().map(Square::getNotationDrives).collect(Collectors.joining(","))).collect(Collectors.joining(";"));
...
////    String stringStream = diagonals.stream().map(squares -> squares.stream().map(Square::getNotationDrives).collect(Collectors.joining(","))).collect(Collectors.joining(";"));
...
//      if (square.getNotationDrives().equals("a1") || square.getNotationDrives().equals("h8")) {
//        assertEquals(1, square.getDiagonals().size());
//      } else {
//        assertEquals(2, square.getDiagonals().size());
//      }
//    });
////    Map<Integer, List<List<List<String>>>> collect = boardBox.getWhiteDraughts().stream().map(draught -> draught.getDiagonals()).map(lists -> lists.stream().map(squares -> squares.stream().map(square -> square.getNotationDrives()).collect(Collectors.toList())).collect(Collectors.toList())).collect(Collectors.groupingBy(o -> o.size()));
...
////    collect = boardBox.getBlackDraughts().stream().map(draught -> draught.getDiagonals()).map(lists -> lists.stream().map(squares -> squares.stream().map(square -> square.getNotationDrives()).collect(Collectors.toList())).collect(Collectors.toList())).collect(Collectors.groupingBy(o -> o.size()));
...
////     TODO more checks
////    assertTrue(isSubDiagonal(diagonal0, Collections.singletonList(diagonal0.getNotationDrives(2))));
//  }
//
...
//    c3 = BoardUtils.findSquareByNotation(c3.getNotationDrives(), board);
//    assertTrue(c3.isOccupied());
...
//    c3 = BoardUtils.findSquareByNotation(c3.getNotationDrives(), board);
//    assertFalse(c3.isOccupied());
//    d4 = BoardUtils.findSquareByNotation(d4.getNotationDrives(), board);
...
//    e5 = BoardUtils.findSquareByNotation(e5.getNotationDrives(), board);
