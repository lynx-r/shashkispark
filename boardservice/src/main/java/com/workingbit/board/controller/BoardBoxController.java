...
public class BoardBoxController {
  ...
  public static Route findBoardById = (req, res) ->
      ((ModelHandlerFunc<DomainId>) (params, token) ->
          Answer.ok(boardBoxService.findById(params, token))
      ).handleRequest(req, res, Authority.BOARD_BY_ID, DomainId.class);

  public static Route findBoardByIds = (req, res) ->
      ((ModelHandlerFunc<DomainId>) (articleId, token) ->
          Answer.ok(boardBoxService.findByArticle(articleId, token))
      ).handleRequest(req, res, Authority.BOARD_BY_ARTICLE, DomainId.class);

  public static Route findPublicBoardByArticleId = (req, res) ->
      ((ModelHandlerFunc<DomainId>) (articleId, token) ->
          Answer.ok(boardBoxService.findPublicByArticleId(articleId, token))
      ).handleRequest(req, res, Authority.BOARD_PUBLIC_BY_ARTICLE_ID, DomainId.class);
